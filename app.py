...
with st.expander("📖 מדריך שימוש"):
    st.markdown("""
**מה האפליקציה עושה?**
עריכת כל הגרפים במצגת PowerPoint דרך קובץ אקסל אחד — גיליון נפרד לכל גרף.

**איך זה עובד?**

1. **העלאת מצגת** — בחרו קובץ `.pptx`, או נסו את המצגת לדוגמה.
2. **הורדת אקסל** — יורד קובץ אקסל ובו כל הגרפים שזוהו במצגת, גיליון לכל גרף.
3. **עריכה באקסל** — שנו את הערכים, שמות הסדרות והקטגוריות. שמרו את הקובץ.
4. **העלאת האקסל המתוקן** — חזרו לאפליקציה והעלו את הקובץ הערוך.
5. **הורדת המצגת המעודכנת** — תקבלו קובץ `.pptx` חדש עם הנתונים המעודכנים.

**מה חשוב לדעת?**

- העיצוב של המצגת והגרפים נשמר — רק הנתונים מתעדכנים.
- אין לשנות את שמות הגיליונות באקסל — הם משמשים לזיהוי הגרפים.
- ניתן להוסיף שורות (קטגוריות) או לשנות ערכים, אך לא להפחית את מספר העמודות המקוריות.
- גיליונות שלא תואמים לגרף קיים יסומנו באזהרה וידולגו.
""")
